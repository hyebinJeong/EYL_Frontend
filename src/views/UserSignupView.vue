      ...
      <div class="bg-white p-8 rounded-lg w-full max-w-md">
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          ...
          <div>
            ...
            <p v-if="name">
              <span v-if="!nameValid" class="text-red-500 text-sm">이름은 2자 이상이어야 합니다.</span>
              <span v-else class="text-green-600 text-sm">사용 가능한 이름입니다.</span>
            </p>
          </div>
          ...
          <div>
            ...
            <div class="flex space-x-2 mt-1">
              ...
              <button type="button"
                ...
                @click="checkNicknameDuplicate">중복검사</button>
            </div>
            <p v-if="nickname">
              <span v-if="!nicknameValid" class="text-red-500 text-sm">닉네임은 2자 이상이어야 합니다.</span>
              <span v-else class="text-green-600 text-sm">사용 가능한 닉네임입니다.</span>
            </p>
          </div>
          ...
          <div>
            ...
            <div class="flex space-x-2 mt-1">
              ...
              <button type="button"
                class="w-24 bg-red-600 hover:bg-red-700 text-white rounded-full font-semibold"
                @click="checkUsernameDuplicate">중복검사</button>
            </div>
            <p v-if="username">
              <span v-if="!usernameValid" class="text-red-500 text-sm">유효한 이메일 형식을 입력하세요.</span>
              <span v-else class="text-green-600 text-sm">사용 가능한 이메일입니다.</span>
            </p>
          </div>
          ...
          <div>
            ...
            <input v-model="password" type="password"
              class="w-full border border-gray-400 rounded-full px-4 py-2 outline-none" />
            <p v-if="password">
              <span v-if="!passwordValid" class="text-red-500 text-sm">비밀번호는 6자 이상이어야 합니다.</span>
              <span v-else class="text-green-600 text-sm">사용 가능한 비밀번호입니다.</span>
            </p>
          </div>
          ...
          <div>
            ...
            <p v-if="passwordConfirm">
              <span v-if="!passwordsMatch" class="text-red-500 text-sm">비밀번호가 일치하지 않습니다.</span>
              <span v-else class="text-green-600 text-sm">비밀번호가 일치합니다.</span>
            </p>
          </div>
          ...
          <div>
            ...
            <input v-model="phone" type="tel" maxlength="13" @input="onPhoneInput"
              class="w-full border border-gray-400 rounded-full px-4 py-2 outline-none"
              placeholder="숫자만 입력하세요" />
            <p v-if="phone">
              <span v-if="!phoneValid" class="text-red-500 text-sm">전화번호 형식이 올바르지 않습니다.</span>
              <span v-else class="text-green-600 text-sm">올바른 전화번호 형식입니다.</span>
            </p>
          </div>
...
const usernameValid = computed(() => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(username.value)) // 이메일 형식
